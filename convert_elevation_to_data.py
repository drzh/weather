...
with open(fi, 'r') as f:
    ...
    dataall = pd.merge(latlon, dataall, on=['lat', 'lon'], how='left')
    dataall = dataall.fillna(0)

    if latlon.shape[0] == dataall.shape[0]:
        vec = np.array(dataall['data'], dtype=np.int32)
        dataout = [latbs, lonbs, vec]
        with lzma.open(fo, 'wb') as f:
            pickle.dump(dataout, f)
    else :
        print(fi, 'is NA ', dataall.shape, file = sys.stderr)
